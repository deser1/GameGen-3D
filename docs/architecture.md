...
### 3. Generowanie Wielowidokowe z Kontrolą Kształtu
* **Moduł:** `TextToMultiViewGenerator` (Stable Diffusion v1.5 + ControlNet Canny).
* **Działanie:** Na bazie wyciętego obrazu i promptu, model generuje zsynchronizowane widoki ortograficzne obiektu. ControlNet pilnuje, by sylwetka była ściśle zachowana, co eliminuje halucynacje 3D. System obsługuje również wstrzykiwanie stylów (np. Voxel, Low Poly).

### 4. Błyskawiczna Rekonstrukcja LRM
...
### 5. Optymalizacja pod Silniki Gier (Mesh, UV, PBR, LOD, Collision, Rigging)
...
### 7. VLM Art Director (Ocena Jakości)
